...
class Disposisi extends CI_Controller {
    ...
    public function index(){
        if ($this->session->userdata('level_user')=="kepala sekolah") {
    ...
    public function simpan(){
        ...
        if($result>=0){
            $datauser = $this->M_user->get_where_default("LEFT JOIN tb_pegawai ON tb_pegawai.nip=tb_user.nip_user WHERE id_bagian_pegawai='$data[id_bagian]' or level_user = 'kepala sekolah'")->result();
    ...
    public function lembar_disposisi($id){
        $row = $this->M_disposisi->get_by_id($id);
        $kepaladesa= $this->M_user->getwhere(array('level_user' => "kepala sekolah", ))->row();
        if ($row) {
